...
up_genes_control_13 <- res_df_control_13 %>% 
  dplyr::filter(log2FoldChange > 1, padj < 0.00001) %>% 
  dplyr::arrange(padj) %>%
  dplyr::slice_head(n=15) #limit number of labeled genes to top 15 up-regulated genes. without it, too many genes are labeled and not readable.

# filter downregulated gene by p value of 1e-5, save as variable
down_genes_control_13 <- res_df_control_13 %>% 
  dplyr::filter(log2FoldChange < -1, padj < 0.00001) %>%
  dplyr::arrange(padj) %>%
  dplyr::slice_head(n=15) #limit number of labeled genes to top 15 down-regulated genes. without it, too many genes are labeled and not readable.
